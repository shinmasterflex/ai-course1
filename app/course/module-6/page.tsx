...
export default function Module6Page() {
  ...
  const handleReturnToDashboard = () => {
    const finalSection = sections[2]
    if (finalSection) {
      markSectionComplete(MODULE_ID, finalSection.id)
      setCurrentPosition(MODULE_ID, finalSection.id)
    }
        ...
        <main className="flex-1 p-8 max-w-4xl mx-auto">
          ...
          {currentSectionIndex === 2 && (
            <div className="space-y-6" id="daily-call-sheet">
              ...
              <Button onClick={handleReturnToDashboard} className="bg-brand-green hover:bg-[#143d31] text-white">
                Return to Dashboard
              </Button>
            </div>
          )}
