...
export const COURSE_CONTENT_REGISTRY: Record<string, CourseContentEntry> = {
  ...
  "module-1-myths-vs-reality": {
    ...
    cardType: "section-card",
    content: "`A number of misconceptions continue to shape how businesses evaluate artificial intelligence.\n\nOne of the most common is the belief that AI “understands” the tasks it performs. In reality, most modern AI systems operate by identifying statistical patterns in data and generating likely outputs rather than comprehending meaning in a human sense. While these systems can produce remarkably sophisticated results, their capabilities are fundamentally predictive rather than conscious or interpretive.\n\nAnother widespread assumption is that AI will immediately replace large portions of the workforce. Although AI is capable of automating many repetitive or narrowly defined tasks, task automation should not be confused with full job elimination. Most occupations consist of multiple responsibilities, only some of which are susceptible to automation. As a result, AI more often restructures work than eliminates it entirely.\n\nBusinesses also frequently assume that AI systems are inherently objective. However, AI models are trained on human-generated data, which means they can reproduce or even amplify existing biases present in that data. Outcomes therefore depend heavily on the quality, diversity, and design of the training process, as well as the safeguards implemented during deployment.\n\nA further misconception is that more expensive or more advanced AI systems are automatically superior. In practice, the effectiveness of an AI solution depends less on raw computational capability and more on how well the system integrates into a company’s workflow, operational goals, and decision-making processes. A simpler model that aligns closely with business needs may provide greater value than a more sophisticated but poorly integrated alternative.\n\nFinally, many organizations believe they should delay adoption until the “right moment” arrives. Yet postponing experimentation and implementation can carry significant long-term costs. Because AI capabilities and organizational learning compound over time, firms that delay adoption may fall behind competitors that are already building expertise, refining workflows, and accumulating operational advantages.",
  ...
  "module-1-choosing-tools-section-card": {
    ...
    sectionId: "evaluation",
    cardType: "section-card",
    content: "Choosing AI tools requires matching capability to workflow needs rather than chasing benchmark performance alone.\n\nOne of the most important considerations is whether the tool integrates effectively with existing systems and operational processes. Even highly capable AI systems can create inefficiencies if they require major workflow disruptions or incompatible infrastructure changes.\n\nOrganizations must also evaluate how the system handles data. This includes understanding what information the tool requires, what data it stores, how it transmits information, and whether its practices align with the organization’s security, compliance, and privacy requirements.\n\nAnother critical factor is the total cost of ownership. Beyond initial subscription or licensing fees, businesses must consider implementation costs, employee training, maintenance requirements, scaling expenses, and the operational costs associated with monitoring and managing the system over time.\n\nError handling and system reliability are equally important. AI systems inevitably produce incorrect or inconsistent outputs under certain conditions, making it essential to understand how failures are detected, communicated, and mitigated within operational environments.\n\nVendor stability should also be considered during evaluation. Since many AI tools depend on ongoing updates, infrastructure support, and long-term platform viability, selecting vendors with strong financial and operational foundations can reduce future disruption risk.\n\nMost AI tool selection mistakes occur because organizations evaluate products based primarily on demonstrations rather than operational fit. While demos often showcase idealized performance under controlled conditions, successful implementation depends on how effectively a system functions within real business constraints, workflows, and organizational requirements.",
